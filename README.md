...
|:-------:|:---------:|
| `:4.104.2`, `:latest` | [View](variants/4.104.2) |
| `:4.104.2-docker` | [View](variants/4.104.2-docker) |
| `:4.104.2-docker-go-1.20.14` | [View](variants/4.104.2-docker-go-1.20.14) |
| `:4.104.2-docker-rootless` | [View](variants/4.104.2-docker-rootless) |
| `:4.104.2-docker-rootless-go-1.20.14` | [View](variants/4.104.2-docker-rootless-go-1.20.14) |
...
Base variants are based on `alpine`, and include `npm 8` and `nodejs 16` (to run `code-server`), `pwsh`, and basic tools. E.g. `4.104.2`
...
```sh
docker run --name code-server --rm -it -p 127.0.0.1:8080:8080 theohbrothers/docker-code-server:4.104.2
...
```sh
docker run --name code-server --rm -it -p 127.0.0.1:8080:8080 theohbrothers/docker-code-server:4.104.2 --bind-addr=0.0.0.0:8080 --auth=none --disable-telemetry --disable-update-check
```

### `docker` variant(s)

```sh
docker run --name code-server --rm -it --privileged -v docker:/var/lib/docker -p 127.0.0.1:8080:8080 theohbrothers/docker-code-server:4.104.2-docker
# code-server is now available at http://127.0.0.1:8080. To login, use the password in the config file:
docker exec code-server sh -c 'cat ~/.config/code-server/config.yaml'
```
...
```sh
docker run --name code-server --rm -it -p 127.0.0.1:8080:8080 theohbrothers/docker-code-server:4.104.2-docker --bind-addr=0.0.0.0:8080 --auth=none --disable-telemetry --disable-update-check
```
...
```sh
docker run --name code-server --rm -it --privileged -p 127.0.0.1:8080:8080 theohbrothers/docker-code-server:4.104.2-docker-rootless
# code-server is now available at http://127.0.0.1:8080. To login, use the password in the config file:
docker exec code-server sh -c 'cat ~/.config/code-server/config.yaml'
```
...
```sh
docker run --name code-server --rm -it -p 127.0.0.1:8080:8080 theohbrothers/docker-code-server:4.104.2-docker-rootless --bind-addr=0.0.0.0:8080 --auth=none --disable-telemetry --disable-update-check
```
